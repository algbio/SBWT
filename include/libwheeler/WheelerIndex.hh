...
public:
    ...
    string get_outlabels() const{
        string S;
        for(int64_t i = 0; i < (int64_t)outlabels.size(); i++) S += outlabels[i];
        return S;   
    }
    ...
    */ 
    ...
    int64_t edge_destination(int64_t edge) const{
        assert(edge >= 0 && edge < (int64_t)outlabels.size());
    ...
    int64_t edge_source(int64_t edge) const{
        ...
        int64_t c = edge_label(edge);
        int64_t outlabels_idx = outlabels_select(edge - C[c] + 1, c);
    ...
    pair<int64_t, int64_t> outlabel_range(int64_t node) const{
        ...
        int64_t outdegree = 0;
        while(outdegs_idx + outdegree + 1 <= (int64_t)outdegs.size()-1 && outdegs[outdegs_idx + outdegree + 1] == 0)
            outdegree++;
    ...
    pair<int64_t, int64_t> inedge_range(int64_t node) const{
        if(node == -1) return {1,0};
        int64_t indegs_idx = indegs_select1(node + 1);
        int64_t indegree = this->indegree(node); // Todo optimize: can save one select by reusing indegs_idx
        if(indegree == 0) return {1,0};
        int64_t start = indegs_rank0(indegs_idx);
    ...
    int64_t indegree(int64_t node) const{
        ...
        int64_t ans = 0;
        while(idx < (int64_t)indegs.size() && indegs[idx] == 0){
            idx++;
            ans++;
        }
        return ans;
    ...
    int64_t outdegree(int64_t node) const{
        ...
        int64_t ans = 0;
        while(idx < (int64_t)outdegs.size() && outdegs[idx] == 0){
            idx++;
            ans++;
        }
        return ans;
    ...
    char incoming_character(int64_t node) const{ // todo: test
        ...
        if(pos_in_indegs == (int64_t)indegs.size()-1 || indegs[pos_in_indegs+1] == 1){
            // Indegree zero.
            return '\0';
        } 
     ...
     */
    int64_t outedge_index_to_wheeler_rank(int64_t idx) const{
    ...
    string to_string() const{ // For debugging
        ...
        int64_t next = 0;
        for(int64_t i = 0; i < outdegs.size(); i++){
...
bool operator==(const WheelerIndex<bitvector_t>& wi1, const WheelerIndex<bitvector_t>& wi2){
    ...
    for(int64_t i = 0; i <= (int64_t)wi1.indegs.size(); i++){
    ...
    for(int64_t i = 1; i <= (int64_t)wi1.indegs.size() - wi1.number_of_nodes(); i++){
    ...
    for(int64_t i = 1; i <= (int64_t)wi1.outdegs.size() - wi1.number_of_nodes(); i++){
