...
public:
    ...
    inline static const int64_t VERSION = 1; // Increment this after non-breaking changes
    ...
    BOSS(const string& outlabels_string, const vector<bool>& indegs, const vector<bool>& outdegs, int64_t k) {
        ...
        for(int64_t i = 0; i < (int64_t)indegs.size(); i++) sdsl_indegs[i] = indegs[i];
        for(int64_t i = 0; i < (int64_t)outdegs.size(); i++) sdsl_outdegs[i] = outdegs[i];

        vector<int64_t> counts(256);
        for(char c : outlabels_string) counts[c]++;
        vector<int64_t> C = char_counts_to_C_array(counts);
    ...
    set<string> get_full_edgemers() const{
        set<string> edgemers;
        for(int64_t node = 0; node < Base::number_of_nodes(); node++){
            string label = get_node_label(node);
            if((int64_t)label.size() == k){
    ...
    int64_t edgemer_string_to_edge_wheeler_rank(const string& edgemer) const{
        int64_t node = find_kmer(edgemer.c_str()); // Searches first k chars
        assert(node != -1);
        char c = edgemer.back();
        int64_t edge_idx = Base::outdegs_rank0(Base::outdegs_select1(node+1));
    ...
    bool has_exactly_one_source_node() const{
        ...
        for(int64_t node = 1; node < Base::number_of_nodes(); node++){
    ...
    vector<bool> get_dummy_node_marks() const{
        ...
        while(!dfs_stack.empty()){
            ...
            if(d < k-1){ // Push children
                int64_t out_l, out_r; tie(out_l, out_r) = Base::outlabel_range(v);
                for(int64_t i = out_l; i <= out_r; i++){
                    int64_t edge_wr = Base::outedge_index_to_wheeler_rank(i);
    ...
    vector<int64_t> char_counts_to_C_array(const vector<int64_t>& counts){
    ...
    set<string> get_all_edgemers() const{
        set<string> ans;
        for(int64_t v = 0; v < Base::number_of_nodes(); v++){
